...
c-------------------------------------------------------------
      ...
      real*8 t1,t2,sp,mff,mii,xs,xt,xu
...
c WZ box:
      ...
      vam=0d0
      boxt_wz=boxt_v1v2(t1,sp,mff,mii,mff,mw,mii,mz,vap,vam)
c ZW box:
      vf12=xvf+xaf
      vi34=xvi+xai
      af12=vf12
      ...
      vap=vf12*vi34+af12*ai34
      vam=0d0
      boxt_zw=boxt_v1v2(t1,sp,mff,mii,mff,mz,mii,mw,vap,vam)
c WZ box (crossed):
      vf12=xvf+xaf
      vi34=xvis+xais
      af12=vf12
      ...
      vap=vf12*vi34+af12*ai34
      vam=0d0
      boxu_wz=boxu_v1v2(t2,sp,mff,mii,mff,mw,mii,mz,vap,vam)
c ZW box (crossed):
      vf12=xvfs+xafs
      vi34=xvi+xai
      af12=vf12
      ...
      vap=vf12*vi34+af12*ai34
      vam=0d0
      boxu_zw=boxu_v1v2(t2,sp,mff,mii,mff,mz,mii,mw,vap,vam)
...
c-------------------------------------------------------------
      ...
      real*8 t1,t2,sp,den,denz,mff,mii,xs,xt,xu
...
c      if (mz.gt.0d0) goto 999
     ...
     $     (v12*v34-a12*a34)
      if (xqi.lt.0d0)
     $     box_ww=boxt_v1v2(t1,sp,mff,mii,1d-5,mw,mii,mw,vap,vam)
      if (xqi.gt.0d0)
     $     box_ww=boxu_v1v2(t2,sp,mff,mii,1d-5,mw,mii,mw,vap,vam)
c 999  continue
c ZZ box:
      boxt_zz=0d0
      boxu_zz=0d0
c      if (mz.gt.0d0) goto 999
     ...
     $     (v12*v34+a12*a34)
      vam=(vf12*vi34-af12*ai34)*(sp-mz**2)/den+xqf*xqi/sp*
     $     (v12*v34-a12*a34)
      boxt_zz=boxt_v1v2(t1,sp,mff,mii,mff,mz,mii,mz,vap,vam)
c ZZ box (crossed):
      boxu_zz=boxu_v1v2(t2,sp,mff,mii,mff,mz,mii,mz,vap,vam)
c 999  continue
c sum of box diagrams
...
*
c weak contribution:
c formfactors describing the 
c weak contribution to the (Z,gamma)ffbar vertex correction.
c vertex: i lamda_mu(s) = i*e*g_mu*(fv(s)-ga(s) g5)
*
      implicit real*8(a-z)
      integer i
      complex*16 b0,b1,c0,spence,cscal,ieps
...
c 1-loop electroweak and higher order QCD (+resummation); 
c gammaloop = gammatree*(1+corr.)
c as taken from Wolf's PhD thesis !
*
      implicit real*8(a-z)
      integer f,fp,rep,qcd,mzero,nlf
...
c 1-loop electroweak and higher order QCD; 
c gammaloop = gammatree*(1+corr.)
*
      implicit real*8(a-z)
      integer f,fp,rep,qcd,hqcd,twol,mix,mzero
...
*
c calculation of the hadronic vacuum polarisation
c for 5 active flavours:
*
      implicit real*8(a-z)
      integer ff
*
...
c-------------------------------------------------------------
      real*8 function a2qqz_all(xs,xt,xu,xvf,xaf,xqf,
     $     xvi,xai,xqi,sigborn)
c-------------------------------------------------------------
c calculation of the weak 1-loop corrections to the Z-exchange
c process:  i(p4)+is(p3)->Z(s=q^2)/gamma->f(p2)+fs(p1).
c from nutev code
c-------------------------------------------------------------
      implicit none
      integer i
      real*8 boxq,box_ww,boxt_zz,boxu_zz
      real*8 boxt2_v1v2,boxu2_v1v2
      real*8 tp,up,sp,den,denz,xs,xt,xu
      real*8 xvf,xaf,xqf,xvi,xai,xqi,kap1,kap2
      real*8 xmi,mi
      real*8 vf,af,qf,vi,ai,qi
      real*8 sigz_weak,siggz_weak,sigq_weak,sigq_weak_all
      real*8 vfs,afs,qfs,vis,ais,qis,xm,xnc
      real*8 alaqzz,blbqzz,alaqgz,blbqgz
      real*8 mue,mue2,mue4
      real*8 dz2z,dz2g,dz2gz,dsigzz
      real*8 rs,mw,mz,xmz,xmw,gamz,gamw
      real*8 pi,pi2,w2,gfermi,alpha0,alpha,sw,cw,mh,xmh,mtop
      real*8 sw2,cw2
      real*8 pig0,sgz0
      real*8 a2qqz_qed,a2qqz_weak,sigborn
      real*8 born,a2qqz
      complex*16 ieps,zero
      complex*16 rsiggg,rsiggz
      complex*16 dsiggz,dsiggg,sigzz,siggg,siggz,sigzos
...
c-------------------------------------------------------------
c matrix element squared with weak vertex and self energy corrections
c to the Z/gamma exchange process:
c (all fermions are considered to be massless !)
c-------------------------------------------------------------
      ieps=dcmplx(0d0,1d-8)
      zero=dcmplx(0d0,0d0)
...
c Z boson propagator:
      denz=sp-xmz
c renormalised gauge bsoson self energies:
c check of mue-dependence, ok
c      do i=1,3
c         mue=1d1**(i**2)
c         mue2=mue*mue
c         mue4=mue2*mue2
c         call zrenorm(dz2z)
c         dz2g=-pig0
c         dz2gz=cw*sw/(cw2-sw2)*(dz2z-dz2g)
*
      rsiggg=siggg(tp)+tp*dz2g
      rsiggz=siggz(tp)-sgz0-tp*dz2gz
      if (dabs(tp).lt.1d-10) then
         piggs=0d0
         pigzs=dsiggz(0d0)-dz2gz
      else
         piggs=rsiggg/tp
      ...
      pizzs=(sigzz(tp)-dreal(sigzos)+denz*dz2z)/denz
      pizs=pizzs-rsiggz**2/(tp+rsiggg)/denz
*
c renormalised vertex functions:
      call prop(4,xm,qfs,vfs,afs,xnc)
      if (qi.gt.0d0)
     $     call prop(10,xm,qis,vis,ais,xnc)
      if (qi.lt.0d0)
     $     call prop(7,xm,qis,vis,ais,xnc)
      call formgz(tp,vf,af,qf,vfs,afs,qfs,fvzf,fvgf,gazf,gagf)
      call formgz(tp,vi,ai,qi,vis,ais,qis,fvzi,fvgi,gazi,gagi)
*
c      fvzf=zero
c      gazf=zero
c      fvgf=zero
...
c      stop
c     
      alaqzz=2d0*(vi**2+ai**2)*(vf*dreal(fvzf)+af*dreal(gazf)-
     $     (vf**2+af**2)*dreal(pizs))+
     $     2d0*(vf**2+af**2)*(vi*dreal(fvzi)+ai*dreal(gazi))
      blbqzz=4d0*vi*ai*(af*dreal(fvzf)+vf*dreal(gazf)-
     $     2d0*vf*af*dreal(pizs))+
     $     4d0*vf*af*(ai*dreal(fvzi)+vi*dreal(gazi))
      sigz_weak=8d0*(alaqzz*(up**2+sp**2)+blbqzz*(sp**2-up**2))
*
      alaqgz=-2d0*vi*qi*(denz*(vf*dreal(fvgf)+af*dreal(gagf))-
     $     (vf**2+af**2)*tp*dreal(pigzs))
      blbqgz=-2d0*ai*qi*(denz*(af*dreal(fvgf)+vf*dreal(gagf))-
     $     2d0*vf*af*tp*dreal(pigzs))
      siggz_weak=8d0*(alaqgz*(up**2+sp**2)+blbqgz*(sp**2-up**2))

      sigq_weak_all=16d0*(sigz_weak+siggz_weak/tp)
*
c-------------------------------------------------------------
c matrix element squared for weak boxes to NC process: f(=nu)+i->f'+i'
c (all fermions are considered to be massless !)
c-------------------------------------------------------------
c WW box:
      kap1=(vf+af)*(vi+ai)/8d0/sw2/sw2
      kap2=0d0
      if (xqi.lt.0d0)
     $     box_ww=boxt2_v1v2(tp,sp,mw,mw,kap1,kap2)
      if (xqi.gt.0d0)
     $     box_ww=boxu2_v1v2(up,sp,mw,mw,kap2,kap1)
c ZZ box:
      kap1=ai*af*(af**2+3d0*vf**2)*(ai**2+3d0*vi**2)+
     $        vi*vf*(vf**2+3d0*af**2)*(vi**2+3d0*ai**2)
      kap2=ai*af*(af**2+3d0*vf**2)*(ai**2+3d0*vi**2)-
     $     vi*vf*(vf**2+3d0*af**2)*(vi**2+3d0*ai**2)
      boxt_zz=boxt2_v1v2(tp,sp,mz,mz,kap1,kap2)
c ZZ box (crossed):
      boxu_zz=boxu2_v1v2(up,sp,mz,mz,kap2,kap1)
 999  continue
c sum of box diagrams
      boxq=alpha0/4d0/pi*32d0*denz*(box_ww+boxt_zz+boxu_zz)
c sum:
 9999 continue
      a2qqz_weak=sigq_weak_all+boxq
c-------------------------------------------------------------
c matrix element squared with photonic vertex corrections
c to the Z exchange process:
c (all fermions are considered to be massless !)
c-------------------------------------------------------------
*
c renormalised vertex functions:
      call formzqed(tp,qi,mi,fvzi_qed)
      a2qqz_qed=2d0*dreal(fvzi_qed)*sigborn
* sum of all virtual contributions:
      a2qqz_all=a2qqz_weak+a2qqz_qed
*
      return
      end
c-------------------------------------------------------------
...
c-------------------------------------------------------------
c full electroweak 1-loop corrections to the 
...
c-------------------------------------------------------------
      ...
      real*8 xvfs,xafs,xvis,xais
      real*8 xqi,xqis,xqfs,xmi,xmis,xmfs
      real*8 qi,qis,qfs,mi,mis,mfs
      real*8 fvweak,fvg,denw,dz2w
      complex*16 swwos,sww,dswwos,dsww,fvwff,fvwii
      complex*16 sigwwp,swwp,swwpos,sigzz,szzos
      complex*16 fvgff,fvgii,fvbox
      complex*16 spence,ieps,zero
      common/cc/ieps
      real*8 pi,pi2,w2
      common/para/pi,pi2,w2
      real*8 sw,cw,sw2,cw2,mh,xmh,mtop,alpha,alpha0,gfermi
      real*8 rs,mw,mz,xmw,xmz,gamw,gamz
      common/weak/sw,cw,sw2,cw2,mh,xmh,mtop,alpha,alpha0,gfermi
      common/par_gen/rs,mw,mz,xmw,xmz,gamz,gamw
      real*8 den
      common/par_prop/den
...
c photonic part to W self energy is included in form factors
      call fvpureg(tp,0d0,qfs,1d-5,mfs,fvgff)      
      call fvpureg(tp,qis,qi,mis,mi,fvgii)
      fvg = dreal(fvgff+fvgii)
c photonic box contribution:
      call fvboxwg_new(iqqbar,tp,sp,qi,qis,qfs,mi,mis,mfs,fvbox)
...
*
      implicit real*8(a-z)
      integer i
      complex*16 b0,b1,c0,C0_,spence,cscal,ieps
      complex*16 factor,fvz,fvg,gaz,gag
...
*
      ...
      real*8 sw2,cw2
      real*8 mue,mue2,mue4
      complex*16 b0,c0,C0_,b0m,b0ms,b0s,ieps,spence
...
*
      implicit real*8(a-z)
      integer i
      complex*16 b0,b1,c0,C0_,spence,ieps,cscal
...
*-----------------------------------------------------------------
      subroutine fvboxwg_new(iqqbar,
     $     xt,xs,xqi,xqis,xqfs,xmi,xmis,xmfs,form)
*
c pure photonic contribution to W exchange box diagrams:
*
      implicit none
      integer iqqbar
      real*8 xt,xs,t,s,u,lwt
      real*8 xqi,xqis,xqfs,xmi,xmis,xmfs,qi,qis,qfs,mi,mis,mfs
      real*8 rs,mw,mz,xmz,xmw,gamz,gamw
      real*8 pi,pi2,w2,gfermi,alpha0,alpha,sw,cw,mh,xmh,mtop
      real*8 sw2,cw2
      complex*16 d0,D0_,C0_,spence,ieps,c0234,c0134,c0123,c0124
...
c      stop
      if(iqqbar.eq.1)
     $     factor(1) = 128d0*qis*qfs*s**2*(s*d0-c0234-c0134)
      if(iqqbar.eq.2)
     $     factor(1) = 64d0*qis*qfs*
...
c      stop
      if(iqqbar.eq.1)
     $     factor(2)=64d0*qi*qfs*(-u*(s**2+u**2+2d0*xmw*u+xmw**2)*d0+
     $     (s**2+u**2+xmw*s+xmw*u)*(c0134+c0234)+
     $     (s*u-u**2-xmw*u)*(c0123+c0124)+2d0*s*(dlog(-xmw/u)+lwt/t))
      if(iqqbar.eq.2) factor(2)=128d0*qi*qfs*u**2*(u*d0-c0234-c0134)
*
...
c change on 12/14/99: -pi-> pi !
         azneut = azneut+
     $        8d0/3d0*nc*a**2*s*dcmplx(-cdlog(sc/mue2)+5d0/3d0,pi)
      end do
