...
*
c specification of the external fermions for the CC process:
c i=qi(p4),i'=qis(p3);f=nu or lepton(p2),f'=lepton+(p1):
*
      mmi = mii
      mmis = mmi
      mmf = 1d-5
...
c with terms prop. to deltas:
                  fqed(j) = betf*le+2d0*deltf+
     $                 betf*deltas*(deltas-4d0)/4d0+alpha0/pi*
     $                 (-dreal(spence(deltas+ieps))+
     $                 (1d0-deltas)*(3d0-deltas)/4d0*
     $                 dlog(1d0-deltas)+deltas*(18d0-deltas)/24d0)
...
*
     ...
     $        pi2/3d0-2d0)+qfs**2*(3d0/2d0*dlog(s/mfs2)+
     $        pi2/3d0-2d0)+3d0+pi2/12d0)
      end if
*
      if (i.eq.3) then
...
*
      implicit real*8(a-z)
      integer i
      complex*16 b0,c0,b0m,b0ms,b0s,ieps,spence
...
*
      implicit real*8(a-z)
      complex*16 b0,b0t,b0u,d0,fvt,fvu,spence,ieps
